...
A standard report can be generated with the option `r I("--report")` when running Kraken2 on the command line; please note that the flag `r I("--report-minimizer-data")`
must also be used. An MPA-style report can be generated when the options `r I("--report")` and `r I("--use-mpa-style")` are combined on the command line; alternatively,
MPA-style reports can be generated from a standard report with the script `r I("kreport2mpa.py")` from the KrakenTools toolkit.
...
The functions `r I("load_MPAreports()")` and `r I("load_STDreports()")` will create dataframes containing all samples that are present in the directory
we specified.\

```{r loading_kraken2_results}
mpa_reports <- load_MPAreports("test/mpa")
std_reports <- load_STDreports("test/reports")
```
...
The function `r I("mergeReports()")` will do the merging task:\
...
To add metadata to our merged dataframe, we can simply use the function `r I("addMetadata()")`, specifying the columns that we want to add
...
* The function `r I("addSampleSize()")` will add a column with the total number of fragments that were analysed by Kraken2 per sample.
* The function `r I("addMinimiserData()")` will add columns with minimiser data from Kraken2's reference database.
...
Note that the columns `r I("sample_size")`, `r I("db_n_minimisers_taxon")`, and `r I("db_n_minimisers_clade")` have been added.\
...
```{r classification_summary_barplot, echo = TRUE, fig.retina = 1, fig.width = 6, fig.height = 2.5, fig.align = 'center'}
...
```{r domain_reads_violin, echo = TRUE, fig.retina = 1, fig.width = 4, fig.height = 2.5, fig.align = 'center'}
...
```{r domain_reads_barplot, echo = TRUE, fig.retina = 1, fig.width = 7, fig.height = 2.25, fig.align = 'center'}
...
Note that in the plots above no eukaryotes were displayed - this happened because we set `r I("include_eukaryotes = FALSE")`.
...
```{r domain_reads_violin_eukaryotes, echo = TRUE, fig.retina = 1, fig.width = 3.5, fig.height = 3, fig.align = 'center'}
plotDomainReads_violin(
...
```{r domain_reads_barplot_eukaryotes, echo = TRUE, fig.retina = 1, fig.width = 7, fig.height = 2.25, fig.align = 'center'}
plotDomainReads_barplot(
...
```{r statistical_assessment, echo = TRUE, fig.retina = 1, fig.width = 25, fig.height = 15, fig.align = 'center'}

merged_reports <- subsetReports(merged_reports, species_to_remove = "Homo sapiens")
